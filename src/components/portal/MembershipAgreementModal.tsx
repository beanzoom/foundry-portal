...
export function MembershipAgreementModal({ open, userFirstName, userLastName, onAgree }: MembershipAgreementModalProps) {
  ...
  // Load Membership Agreement content
  useEffect(() => {
    if (open) {
      fetch('/docs/membership-agreement.md')
        .then(res => res.text())
        .then(text => setAgreementContent(text))
        .catch(err => {
          console.error('Failed to load Membership Agreement:', err);
          setError('Failed to load Membership Agreement document');
        });
    }
  }, [open]);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          console.log('Membership Agreement bottom visible via IntersectionObserver');
    ...
    const handleScroll = () => {
      ...
      if (distanceFromBottom < 100) {
        console.log('Membership Agreement bottom detected via scroll handler');
        setHasScrolledToBottom(true);
      }
    };
      ...
      <DialogContent className="max-w-4xl h-[90vh] p-0 flex flex-col" onPointerDownOutside={(e) => e.preventDefault()}>
        <DialogHeader className="p-6 pb-0">
          <div className="flex items-center gap-2">
            ...
            <DialogTitle className="text-2xl">Membership Agreement</DialogTitle>
          </div>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-hidden p-6 pt-4">
          <Alert className="mb-4">
            ...
            <AlertDescription>
              You must read and agree to this Membership Agreement to access the FleetDRMS Portal.
        ...
        {hasScrolledToBottom && (
          <div className="border-t p-6 space-y-4 bg-gray-50">
            ...
            >
              {loading ? 'Saving Agreement...' : 'I Agree to the Membership Agreement'}
            </Button>
          </div>
        )}
      </DialogContent>
